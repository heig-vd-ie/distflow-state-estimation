...
class DistFlow():
    def __init__(self, line_data: pl.DataFrame, ext_grid_id: str, max_iter: int = 100, tol: float=1e-6):
        ...
        self.z_init_gb: gb.Matrix # type: ignore
        ...
        self.z_init_np: np.array # type: ignore
    ...
    def generate_initial_impedance_matrix(self):
        ...
        self.z_init_gb: gb.Matrix = r_init.ewise_add(1j* x_init) # type: ignore
    ...
    def graphblas_to_numpy(self):
        ...
        self.z_init_np = self.z_init_gb.to_dense(fill_value=0.0)
    ...
    def timeseries_distflow_algorithm(
        ...
        if engine == "numpy":
            ...
            for idx in range(np.shape(s_node)[0]):
                s_flow_gb[idx, :], v_sq_gb[idx, :], i_sq_gb[idx, :] = self.distflow_algorithm_np(s_node_gb[idx, :], v0_sq_gb[idx, :]) 
